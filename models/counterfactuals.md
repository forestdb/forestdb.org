...
This version is an informative speaker trying to comunicate the world state (all top-level vars in a model) to a literal listener. The meaning of "because" is both the presuppositions and the counterfactual. Because the speaker has to consider different explanations we have to dynamically construct the listener query.
