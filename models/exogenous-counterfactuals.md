...
*This forest page discusses issues that arise with the [current counterfactual model](http://forestdb.org/models/because.html) and how these issues can be resolved using exogenous randomness (cf. Pearl, 2000).*
...
### Efficiency
In the above models we see that the exogenous style is significantly slower than the original style. Here the difference between two and twelve seconds is not exceedingly inconvenient. It becomes a serious impediment, however, when we begin using more complex models. For example, below is a simple fish category model taken from [forestdb.org/models/category-explanations](/models/category-explanations.html). It takes around twelve minutes to run. We would like to add pragmatics to the model to generate better predictions, but at the moment, it seems infeasible.
