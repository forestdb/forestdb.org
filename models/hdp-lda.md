...
The code below is only a sketch — it omits the `mh-query` sample-count and lag arguments and refers to an unspecified corpus (`corpus-words`, `corpus-docs`, `corpus-observed-words`) — so it is shown as a static listing rather than a runnable box.
...
The following is a version of the code closer to the one used in Ref:shred2014. It targets the Church dialect used there rather than webchurch, so it is also shown as a static listing:
