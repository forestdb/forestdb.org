...
In the current counterfactual model, causation is only considered in its effect on the correlation between two variables. This is because dependence between the variables of interest is found with a condition statement, not intervention. An introductory statistics professor will tell you that correlation does not imply causation, and that a study must be experimental (with intervention) to demonstrate causality. I argue that this principle applies to counterfactuals just as it does to science. A model that relies entirely on co-occurrence cannot possibly capture causality.
